...
export class AnnouncementSettingsComponent implements OnInit {
  ...
  delete(announcement: Announcement) {
    const dialogRef = this.dialogService.openDeleteDialog('announcement', 'really-delete-announcement', announcement.title);
    dialogRef.afterClosed().subscribe(result => {
      if (result === 'abort') {
        return;
      } else if (result === 'delete') {
        this.announcementService.delete(this.room.id, announcement.id).subscribe(() => {
          const msg = this.translateService.instant('announcement.deleted');
          this.notificationService.showAdvanced(msg, AdvancedSnackBarTypes.WARNING);
          this.announcements = this.announcements.filter(n => n.id !== announcement.id);
        });
      }
    });
  }
  ...
  edit(announcement: Announcement) {
    if (!this.title && !this.body) {
      ...
      const dialogRef = this.dialogService.openDeleteDialog('announcement', 'really-discard-announcement', null, 'discard');
      dialogRef.afterClosed().subscribe(result => {
        if (result === 'abort') {
          return;

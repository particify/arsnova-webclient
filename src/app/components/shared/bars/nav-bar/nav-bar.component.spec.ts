...
import { JsonTranslationLoader, MockGlobalStorageService, MockLangService } from '@arsnova/testing/test-helpers';
import { of } from 'rxjs';
...
describe('NavBarComponent', () => {
  ...
  it('should have navigation buttons for room overview and comments', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    overviewButton = await loader.getHarness(MatButtonHarness.with({selector: '#overview-button'}));
    commentsButton = await loader.getHarness(MatButtonHarness.with({selector: '#comments-button'}));
  ...
  it('should NOT HAVE navigation button for feedback if not enabled and in role PARTICIPANT', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    const feedbackButtonElement = fixture.nativeElement.querySelector('#feedback-button');
  ...
  it('should HAVE navigation button for feedback if locked in role CREATOR', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    feedbackButton = await loader.getHarness(MatButtonHarness.with({selector: '#feedback-button'}));
  ...
  it('should HAVE a user counter if desktop device and creator', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    const userCounter = fixture.nativeElement.querySelector('#user-count-container');
  ...
  it('should navigate to comments when clicking on comments button', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    overviewButton = await loader.getHarness(MatButtonHarness.with({selector: '#comments-button'}));
    await overviewButton.click();
    expect(router.navigate).toHaveBeenCalledWith(['edit', '12345678', 'comments']);
  });
  ...
  it('should navigate to feedback when clicking on feedback button', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    overviewButton = await loader.getHarness(MatButtonHarness.with({selector: '#feedback-button'}));
    await overviewButton.click();
    expect(router.navigate).toHaveBeenCalledWith(['edit', '12345678', 'feedback']);
  });
  ...
  it('should navigate to series when clicking on series button and there is only one series', async () => {
    ...
    await overviewButton.click();
    expect(router.navigate).toHaveBeenCalledWith(['edit', '12345678', 'series', component.groupName]);
  });
  ...
  it('should open menu when clicking on series button and there are multiple series', async () => {
    ...
    seriesMenu = await loader.getHarness(MatMenuHarness.with({selector: '#series-button'}));
  ...
  it('should navigate to series when clicking on series menu item', async () => {
    ...
    await seriesButton.click();
    seriesMenu = await loader.getHarness(MatMenuHarness.with({selector: '#series-button'}));
    seriesMenuItems = await seriesMenu.getItems({selector: '#Test-series-button'});
    await seriesMenuItems[0].click();
    expect(router.navigate).toHaveBeenCalledWith(['edit', '12345678', 'series', 'Test']);
  });
  ...
  it('should add feedback button for participants when feedback has been started', async () => {
    ...
    route.data = of(data);
    fixture.detectChanges();
    let feedbackButtonElement = fixture.nativeElement.querySelector('#feedback-button');
    expect(feedbackButtonElement).toBeNull('Feedback button should be null.');
    const body = {
    ...
    fixture.detectChanges();
    feedbackButtonElement = fixture.nativeElement.querySelector('#feedback-button');
    expect(feedbackButtonElement).not.toBeNull('Feedback button should be NOT NULL.');
  });

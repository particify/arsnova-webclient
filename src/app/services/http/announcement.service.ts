...
export class AnnouncementService extends AbstractHttpService<Announcement> {
  ...
  constructor(private http: HttpClient,
    ...
    super('/announcement', http, eventService, translateService, notificationService);
  }
  ...
  add(roomId: string, title: string, body: string): Observable<Announcement> {
    ...
    return this.http.post<Announcement>(url, announcement).pipe(
      catchError(this.handleError<Announcement>(`Add announcement: ${announcement}`))
    )
  }
  ...
  update(roomId: string, id: string, title: string, body: string): Observable<Announcement> {
    ...
    return this.http.patch<Announcement>(url, changes).pipe(
      catchError(this.handleError<Announcement>(`Patch announcement with id: ${id}`))
    )
  }
  ...
  getByRoomId(roomId: string): Observable<Announcement[]> {
    ...
    return this.http.get<Announcement[]>(url).pipe(
      first(),
      catchError(this.handleError<Announcement[]>('Get announcements by room id'))
    )
  }
  ...
  getStateByUserId(userId: string): Observable<AnnouncementState> {
    ...
    return this.http.get<AnnouncementState>(url).pipe(
      first(),
      catchError(this.handleError<AnnouncementState>('Get announcement state by user id'))
    )
  }
}
